def check_missing_translations
  ...
  if doc = Nokogiri.HTML(open(@browser.url))
    ...
    h.sort{|a,b| a[1]<=>b[1]}.each { |elem|
      puts "<font style=\"BACKGROUND-COLOR: yellow\">Missing translation: <b>#{elem[0]}</b> has <b>#{elem[1]}</b> occurrences on <a href=\"#{@browser.url.to_s}\" target=\"blank\">#{@browser.url.to_s}</a></font>"
   }
  else
    return {}
  end
